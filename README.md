...
| `grouping` | `IFRE_USE_ORTOOLS` | Алгоритм | 2-opt |
|---|---|---|---|
| `true` | любое | Жадный + multi-stop группировка: задачи объединяются в один выезд, порядок оптимизируется | Да |
| `false` | `false` | Жадный без группировки: каждой заявке назначается лучшая свободная машина по очереди | Нет |
| `false` | `true` | OR-Tools VRPTW: глобально оптимальное распределение всех задач по всем машинам сразу | Нет |

**Когда что использовать:**
- `grouping=true` — когда машин меньше, чем заявок; система сама объединит близкие задачи в один выезд
- `grouping=false` + OR-Tools — когда важно оптимально распределить задачи по машинам (минимальный суммарный пробег)
- `grouping=false` без OR-Tools — быстрый baseline для сравнения
